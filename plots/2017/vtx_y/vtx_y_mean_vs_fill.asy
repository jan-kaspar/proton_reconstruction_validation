...
for (int ai : arms.keys)
{
	...
	for (int fi : fills.keys)
	{
		...
		string on = "multiRPPlots/" + arms[ai] + "/p_vtx_y_vs_xi|ff";
	
		RootObject obj = RootGetObject(f, on, error=false);
		if (!obj.valid)
			continue;
	
		real d = obj.rExec("GetParameter", 0) * 1e4;
		real d_unc = obj.rExec("GetParError", 0) * 1e4;
...
GShipout("vtx_y_mean_vs_fill", hSkip=0mm, vSkip=0mm);
