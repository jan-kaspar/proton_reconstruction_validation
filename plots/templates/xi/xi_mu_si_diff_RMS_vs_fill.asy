...
for (int rpi : rps.keys)
{
	...
	NewPad("fill", "RMS of $\xi_{\rm single} - \xi_{\rm multi}$");
	...
	for (int vi : versions.keys)
	{
		...
		for (int xai : xangles_short.keys)
		{
			...
			for (int fi : fills.keys)
			{
				...
				string on = "singleMultiCorrelationPlots/si_rp" + rps[rpi]  + "_mu_" + rp_arms[rpi] + "/g_xi_diff_si_mu_RMS_vs_xi_mu|ff_pol1";
	...
	DrawFillMarkers(0., +0.05);

	limits((-1, 0), (fills.length, +0.05), Crop);
}
