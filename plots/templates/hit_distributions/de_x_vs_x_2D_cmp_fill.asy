...
for (int fi : fills_short.keys)
{
	...
	for (int ai : arms.keys)
	{
		...
		RootObject h2_de_x_vs_x = RootGetObject(f_tracks, "arm " + arms[ai] + "/h2_de_x_vs_x", error=false);

		string f_optics = d + "/output_optics.root";
		RootObject g_disp = RootGetObject(f_optics, "arm " + arms[ai] + "/g_de_x_vs_x_disp", error=false);

		if (!h2_de_x_vs_x.valid || !g_disp.valid)
			continue;

		if (rebin)
			h2_de_x_vs_x.vExec("Rebin2D", 2, 2);
