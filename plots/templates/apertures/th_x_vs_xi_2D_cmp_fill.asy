...
for (int fi : fills_short.keys)
{
	...
	for (int ai : arms.keys)
	{
		...
		RootObject hist = RootGetObject(f_output, "multiRPPlots/arm" + arms[ai] + "/h2_th_x_vs_xi", error=false);

		string f_fit = d + "/do_fits.root";
		RootObject g_aperture = RootGetObject(f_fit, "multiRPPlots/arm" + arms[ai] + "/g_aperture", error=false);
